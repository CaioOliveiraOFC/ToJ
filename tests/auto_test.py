...
class AutoTester:
    ...
    def _create_mocked_safe_get_key(self, real_print):
        ...
        def mocked_safe_get_key(valid_keys=None, allow_escape=True):
            ...
            self._check_victory_condition(level)
            self._update_progress_bar(level, real_print)

            choice = self._determine_bot_choice(valid_keys)
    ...
    def run_test(self, player):
        """Executa o teste automatizado do fluxo do jogo."""
        from src.engine.loop import start_game

        original_draw = self._setup_test_environment(player)
        mocked_draw = self._create_mocked_draw(original_draw)
        mocked_safe_get_key = self._create_mocked_safe_get_key(builtins.print)
        real_print = self._setup_mock_patchers(mocked_safe_get_key, mocked_draw)

        self._execute_simulation(player, real_print, start_game)
